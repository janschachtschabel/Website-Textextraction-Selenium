...
def test_single_and_batch_resolve_same_server_defaults():
    custom = replace(settings, default_mode='fast', default_timeout_seconds=17,
                     default_retries=3, default_max_bytes=9000, default_js_strategy='accuracy')
...
def test_explicit_false_overrides_global_insecure_ssl():
    ...
    assert resolve_options(CrawlRequest(url='https://example.com', allow_insecure_ssl=False), custom).allow_insecure_ssl is False
...
def test_effective_media_policy_and_model_identity_change_cache_key():
    ...
    assert make_cache_key(str(req.url), one, settings) != make_cache_key(str(req.url), one, replace(settings, presidio_de_model='different'))
