...
@pytest.mark.parametrize('address', ['127.0.0.1', '10.1.2.3', '169.254.169.254', '::ffff:127.0.0.1', 'fe80::1', '100.64.0.1'])
def test_private_resolved_targets_are_blocked(address, monkeypatch):
    monkeypatch.setattr(socket, 'getaddrinfo', lambda *a, **k: [(socket.AF_INET6 if ':' in address else socket.AF_INET, socket.SOCK_STREAM, 6, '', (address, 443))])
...
async def test_http_reads_full_document_and_reports_request_limit():
    ...
    transport = httpx.MockTransport(lambda request: httpx.Response(200, content=body, headers={'content-type': 'text/html'}))
...
async def test_retry_is_rate_limited_and_cookies_do_not_cross_requests():
    ...
    def transport(request):
        ...
        return httpx.Response(503 if len(seen) == 1 else 200, content=b'hello',
...
def test_fractional_rate_and_changed_override_share_reservations(tmp_path):
    with diskcache.Cache(str(tmp_path)) as store:
        ...
        assert limiter.reserve('example.com', .5, now=100, latest=200) == 102
        second_worker = RateLimiter(store, settings)
        assert second_worker.reserve('example.com', .5, now=100, latest=200) == 104
...
def test_blocked_status_short_success_and_feed_link_do_not_start_browser():
    ...
    for status, html in [(429, '<div id="root"></div>'), (200, '<main>Short answer.</main>'),
...
def test_empty_spa_shell_needs_browser_but_cookie_words_do_not(article_html):
    ...
    fetched = FetchResult((article_html + '<p>Cookie consent: accept</p>').encode(), 'https://example.com', 200, 'text/html')
