        ...
        with tempfile.TemporaryDirectory(prefix="extract-media-") as directory:
            ...
            proc = subprocess.run(
                [
                    ...
                    "file,pipe",
                    "-format_whitelist",
                    "mov,mp3,wav,ogg,flac,matroska,webm,aac,avi,mpegts",
                    "-show_format",
                    "-show_streams",
                    "-of",
                    "json",
    ...
    *,
    ...
    if ext == ".html":
        return convert_html(
            ...
            content_type,
            url,
            html_converter or settings.html_converter,
    ...
    if mime.startswith(("video/", "audio/")):
        ...
        return ConversionResult(
            status="unsupported",
            warnings=[
                "Legacy full media transcription is disabled: it can invoke unguarded external services; use metadata or skip"
            ],
        )
    ...
    try:
        ...
        return ConversionResult(
            status="failed", warnings=[f"Document conversion failed ({type(exc).__name__}); check format extras"]
        )
