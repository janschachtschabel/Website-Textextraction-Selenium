...
class RateLimiter:
    def __init__(self, store, config: Settings, run_sync=asyncio.to_thread):
        self.store = store
    ...
    async def acquire(self, url: str, rps: float, deadline: Deadline):
        now = time.time()
        slot = await deadline.run(
            self.run_sync(self.reserve, urlsplit(url).hostname, rps, now=now, latest=now + deadline.remaining())
        )
