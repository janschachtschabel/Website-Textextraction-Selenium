    ...
    with _lock:
        if language not in _engines:
            try:
                ...
                import_module(model)
                provider = provider_type(nlp_configuration={
                    'nlp_engine_name': 'spacy', 'models': [{'lang_code': language, 'model_name': model}],
                })
                analyzer = analyzer_module.AnalyzerEngine(nlp_engine=provider.create_engine(), supported_languages=[language])
                _engines[language] = (analyzer, anonymizer_type())
            except Exception as exc:
                raise CrawlError('Anonymization unavailable: install the PII extra and requested language model', 503) from exc
...
def anonymize(text: str, language: str = 'de') -> tuple[str, AnonymizationResult]:
    ...
    try:
        ...
        return result.text, AnonymizationResult(
            entities_found=sorted({item.entity_type for item in entities}), entity_count=len(entities),
        )
    except Exception as exc:
