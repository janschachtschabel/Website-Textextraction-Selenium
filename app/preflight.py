...
_CHALLENGE = re.compile(r'^\s*(?:#{1,6}\s*)?(just a moment|checking your browser|verifying you are human|attention required)', re.I)
...
def needs_browser(fetched: FetchResult, converted: ConversionResult) -> bool:
    ...
    requires_js = bool(re.search(r'(enable javascript|javascript (?:required|wird benötigt)|javascript.*aktivieren)', visible, re.I))
    shell_text = visible.lower().strip(' .…!') in {'', 'loading', 'loading content', 'wird geladen'}
    return ((converted.status != 'ok' or shell_text) and has_script and has_root) or (requires_js and len(visible) < 500)
