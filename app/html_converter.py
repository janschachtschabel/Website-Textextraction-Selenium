...
def markitdown_stream(data: bytes, content_type: str | None, extension: str, url: str | None) -> str:
    ...
    result = _local.markitdown.convert_stream(
        io.BytesIO(data), stream_info=StreamInfo(mimetype=content_type, extension=extension, url=url)
    )
    return result.text_content.strip()


def convert_html(data: bytes, content_type: str | None, url: str | None, converter: str, clean: bool) -> ConversionResult:
    ...
    for candidate in dict.fromkeys(candidates):
        try:
            if candidate == 'trafilatura':
                text = (extract(html, url=url, output_format='markdown', include_links=True,
                                include_tables=True, include_comments=False) if clean else html2txt(html))
