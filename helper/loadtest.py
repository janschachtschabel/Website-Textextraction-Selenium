...
MODES = ["fast", "js", "auto"]  # add "auto" to include preflight mode
CONVERTERS = ["trafilatura", "markitdown"]
CONCURRENCY_LEVELS = [1, 2, 4, 8]
SCREENSHOT_CONCURRENCY_LEVELS = [1, 2, 4]  # JS+screenshot phase (skip conc=8, heavier load)
CACHE_SCENARIO = os.getenv("LOADTEST_CACHE_SCENARIO", "cold")
if CACHE_SCENARIO not in {"cold", "warm"}:
    raise ValueError("LOADTEST_CACHE_SCENARIO must be cold or warm")
...
def classify_error(exc: Exception) -> str:
    ...
    if isinstance(exc, asyncio.TimeoutError) or "Timeout" in name:
        return "timeout"
    if (
        isinstance(exc, aiohttp.ClientConnectorError | aiohttp.ServerDisconnectedError | aiohttp.ClientConnectionError)
        or "Connect" in name
        or "Disconnect" in name
    ):
        return "connection"
...
def response_success(body: dict, screenshot: bool = False) -> bool:
    status = body.get("status_code")
    valid = (
        body.get("success") is True
        and isinstance(status, int)
        and 200 <= status < 300
        and body.get("extraction_status") == "ok"
        and bool(body.get("markdown", "").strip())
        and not body.get("error_page_detected")
        and not body.get("truncated")
    )
    if screenshot:
        try:
            valid = valid and base64.b64decode(body.get("screenshot_base64") or "", validate=True).startswith(
                b"\x89PNG\r\n\x1a\n"
            )
...
async def fetch(
    session, sem, url, mode, converter, concurrency, screenshot=False, group="standard", *, force_refresh=None
):
    payload = {
        "url": url,
        "mode": mode,
        ...
        "force_refresh": CACHE_SCENARIO == "cold" if force_refresh is None else force_refresh,
    }
    ...
    async with sem:
        ...
        try:
            async with session.post(
                API_ENDPOINT, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=REQUEST_TIMEOUT + 10)
            ) as response:
                ...
                success = response_success(body, screenshot)
                cached = bool(body.get("cached"))
                mismatch = group != "warmup" and (cached != (CACHE_SCENARIO == "warm") or bool(body.get("coalesced")))
                if mismatch:
                    success = False
                ...
                return Result(
                    ...
                    error=None
                    if success
                    else ("Cache scenario mismatch" if mismatch else "Extraction or screenshot failed"),
                    error_type=None
                    if success
                    else ("cache_mismatch" if mismatch else body.get("extraction_status", "invalid_result")),
                    error_page_detected=bool(body.get("error_page_detected")),
                    cached=cached,
                    markdown_length=len(body.get("markdown", "")),
                    group=group,
                    screenshot_kb=len(b64) * 0.75 / 1024 if b64 else 0,
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                warming = body.get("status") == "starting"
...
async def run_level(
    mode: str, converter: str, concurrency: int, urls: list[str], screenshot: bool = False, group: str = "standard"
) -> list[Result]:
    ...
    async with aiohttp.ClientSession() as session:
        if CACHE_SCENARIO == "warm":
            await asyncio.gather(
                *(
                    fetch(session, sem, url, mode, converter, concurrency, screenshot, "warmup", force_refresh=True)
                    for url in urls
                )
            )
        tasks = [fetch(session, sem, url, mode, converter, concurrency, screenshot, group) for url in urls]
...
async def run_all() -> list[Result]:
    ...
    print(f"Cache scenario: {CACHE_SCENARIO}; response timing includes the complete body")
    print()
    ...
    for mode in MODES:
        for converter in CONVERTERS:
            for conc in CONCURRENCY_LEVELS:
                step += 1
                label = f"[{step:>2}/{total}] mode={mode:<5} converter={converter:<11} conc={conc:>2}"
                print(f"{label} ...", end=" ", flush=True)
                t0 = time.monotonic()
                results = await run_level(mode, converter, conc, urls, screenshot=False, group="standard")
                wall = time.monotonic() - t0
                ok = [r for r in results if r.success]
                cached = sum(1 for r in ok if r.cached)
                avg = statistics.mean(r.response_time for r in ok) if ok else 0.0
                print(f"ok={len(ok)}/{len(results)}  cached={cached}  avg={avg:.1f}s  wall={wall:.1f}s")
    ...
    for ss_step, conc in enumerate(SCREENSHOT_CONCURRENCY_LEVELS, 1):
        label = f"[{ss_step:>2}/{ss_total}] mode=js    converter=trafilatura  conc={conc:>2}  +screenshot"
        print(f"{label} ...", end=" ", flush=True)
        t0 = time.monotonic()
        results = await run_level("js", "trafilatura", conc, urls, screenshot=True, group="js_screenshot")
        wall = time.monotonic() - t0
        ok = [r for r in results if r.success]
        avg = statistics.mean(r.response_time for r in ok) if ok else 0.0
        ss_ok = [r.screenshot_kb for r in ok if r.screenshot_kb > 0]
        avg_kb = statistics.mean(ss_ok) if ss_ok else 0.0
        print(f"ok={len(ok)}/{len(results)}  avg={avg:.1f}s  wall={wall:.1f}s  png_avg={avg_kb:.0f} KB")
        all_results.extend(results)
...
def plot(results: list[Result], save_path: str) -> None:
    ...
    fig.suptitle(
        f"Load Test – Volltextextraktion Selenium MD\n({n_urls} URLs  x  {len(CONCURRENCY_LEVELS)} concurrency levels)",
        fontsize=13,
        fontweight="bold",
    )
...
def print_summary(results: list[Result], group: str = "standard") -> None:
    ...
    w = 96
    print("\n" + "=" * w)
    print(f"{title} — {CACHE_SCENARIO} cache scenario")
    print(
    ...
    for mode in modes:
        for converter in converters:
            for conc in conc_levels:
                grp = [r for r in subset if r.mode == mode and r.converter == converter and r.concurrency == conc]
                ok_times = [r.response_time for r in grp if r.success]
...
def plot_screenshot_phase(results: list[Result], save_path: str) -> None:
    ...
    ax.grid(True, alpha=0.3)
    ax2 = ax.twinx()
    ax2.bar(SCREENSHOT_CONCURRENCY_LEVELS, err_pcts, width=0.3, alpha=0.25, color="tomato", label="Error %")
    ax2.set_ylabel("Error rate (%)", color="tomato", fontsize=8)
    ax2.set_ylim(0, 105)
    ax2.tick_params(axis="y", labelcolor="tomato")
    plt.tight_layout()
...
def plot_errors(results: list[Result], save_path: str) -> None:
    ...
    for etype in all_types:
        ...
        ax_top.bar(x, heights, bottom=bottoms, label=etype, color=type_colors[etype], alpha=0.85)
        bottoms += heights
    ...
    for etype in all_types:
        heights2 = np.array([conc_data[c].get(etype, 0) for c in CONCURRENCY_LEVELS], dtype=float)
        ax_bot.bar(x2, heights2, bottom=bottoms2, label=etype, color=type_colors[etype], alpha=0.85)
        bottoms2 += heights2
...
ERROR_PATTERNS: dict[str, list[str]] = {
    ...
    "gone": ["410", "removed", "no longer available", "nicht mehr verfügbar", "deleted", "gelöscht"],
    "no_content": [
        "no text content",
    ...
    "redirect_error": ["too many redirects", "redirect loop", "circular redirect", "redirect chain"],
}
...
def classify_patterns(text: str) -> list[str]:
    ...
    if not text:
        return []
    lower = text.lower()
    return [name for name, keywords in ERROR_PATTERNS.items() if any(kw in lower for kw in keywords)]
...
def plot_patterns(results: list[Result], save_path: str) -> None:
    ...
    fig.suptitle("Root-Cause Pattern Distribution per Mode+Converter", fontsize=13, fontweight="bold")

    x = np.arange(len(combinations))
    bottoms = np.zeros(len(combinations))
    for pattern in all_patterns:
        heights = np.array([combo_counts[c].get(pattern, 0) for c in combinations], dtype=float)
        ax.bar(x, heights, bottom=bottoms, label=pattern, color=colors[pattern], alpha=0.85)
        bottoms += heights
...
if __name__ == "__main__":
    ts = CACHE_SCENARIO + "_" + datetime.now().strftime("%Y%m%d_%H%M%S")
